...
def dict():
    ...
    synonym.config(text=', '.join(dictionary.synonym('en', word.get())))
    antonym.config(text=', '.join(dictionary.antonym('en', word.get())))


Label(root, text="Dictionary", font="Arial 36 bold", fg="Light green").pack(pady=10)
frame = Frame(root)
